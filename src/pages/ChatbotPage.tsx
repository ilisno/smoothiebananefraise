...
const ChatbotPage: React.FC = () => {
  ...
  useEffect(() => {
    if (userEmail && !isInitialPopupOpen) {
        ...
        if (messages.length === 0) {
             addMessage('assistant', "Bienvenue ! Pose-moi tes questions sur la musculation. Je serai concis. Si tu cherches un programme complet, je te recommande notre Générateur de Programme !");
        }
    }
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    const systemPrompt = `Tu es un coach musculation virtuel. Donne des conseils pratiques et adaptés. Tes réponses doivent être concises et aller droit au but. 
Si l'utilisateur demande explicitement un programme d'entraînement complet, un plan d'entraînement, ou quelque chose de similaire, réponds que pour un programme personnalisé et complet, il est préférable d'utiliser notre outil dédié. 
Suggère d'utiliser le "Générateur de Programme" et fournis ce lien direct vers celui-ci : [Générateur de Programme Personnalisé](/) . 
Pour toutes les autres questions (exercices spécifiques, nutrition, conseils généraux), réponds directement.`;
